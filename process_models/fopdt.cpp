                              ...
                              double stepTime)
{
    ...
    double initialTemperature = temperatureSamples.front();
    for (size_t i = 0; i < timeSamples.size(); ++i)
    {
        if (timeSamples[i] >= stepTime)
        {
            if (i > 0) initialTemperature = temperatureSamples[i-1];
            break;
        }
    }

    double finalTemperature = temperatureSamples.back();
    ...
    if (std::abs(dutyChange) < 1e-6) return params;
    ...
    for (size_t i = 0; i < timeSamples.size(); ++i)
    {
        if (timeSamples[i] < stepTime) continue;
        
        double currentTemp = temperatureSamples[i];
        
        if (!found28)
        {
            if ((increasing && currentTemp >= tempAt28Percent) || (!increasing && currentTemp <= tempAt28Percent))
            {
                timeAt28Percent = (i > 0) ? core::linearInterpolateX(tempAt28Percent, 
                                                                     timeSamples[i-1], temperatureSamples[i-1], 
                                                                     timeSamples[i], temperatureSamples[i]) : timeSamples[i];
                found28 = true;
            }
        }
        ...
        if (!found63)
        {
            if ((increasing && currentTemp >= tempAt63Percent) || (!increasing && currentTemp <= tempAt63Percent))
            {
                timeAt63Percent = (i > 0) ? core::linearInterpolateX(tempAt63Percent, 
                                                                     timeSamples[i-1], temperatureSamples[i-1], 
                                                                     timeSamples[i], temperatureSamples[i]) : timeSamples[i];
                found63 = true;
            }
